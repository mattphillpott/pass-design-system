      ...
      <div style={{ ...pfsStyles.inner1200, padding: '0 32px', display: 'block' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20, alignItems: 'stretch' }} className="pfs-routes-grid">
          {routes.map((r, i) => {
            ...
            // Flowbite-style light alt badges (info=blue, warning=amber)
            // Map by tag text so order changes don't break colours.
            const altBadge = r.tag === 'TRADITIONAL ROUTE'
              ? { bg: '#EEF6FF', fg: '#1C64F2', border: '#C3DDFD' }   // info / blue
              : r.tag === 'ALTERNATIVE ROUTE'
              ? { bg: '#FFF7ED', fg: '#D97706', border: '#FDE3B7' }   // warning / amber
              : { bg: '#F3F4F6', fg: '#6A7282', border: '#E5E7EB' };  // fallback neutral
            return (
              <div key={r.title} style={{
                ...
                {!isPopular && (
                  <div style={{ alignSelf: 'flex-start', display: 'inline-flex', fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: altBadge.fg, background: altBadge.bg, border: `1px solid ${altBadge.border}`, padding: '5px 10px', borderRadius: 8, marginBottom: 12 }}>{r.tag}</div>
                )}
